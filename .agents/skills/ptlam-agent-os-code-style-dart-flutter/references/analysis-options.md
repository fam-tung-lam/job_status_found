...
how to silence a diagnostic without hiding it.

Keep one `analysis_options.yaml` per package, and keep the existing one when the
project has it. This file describes the baseline, not the resolved
configuration.

## Start from a published rule set

Include a maintained set instead of listing rules by hand:
...
published set keeps it. Do not stack two sets: the last `include` wins and the
earlier one quietly stops applying.

Add or remove single rules under `linter: rules:` below the include, and write
the reason for each removal beside it. The analyzer rejects an unknown name with
`undefined_lint` and a contradictory pair with `incompatible_lint`. Fix both in
the configuration, not the code.

## Turn on the three strict modes
...
three on for a new package. In an existing package, turn them on one at a time
so each batch of failures has one cause.

## Promote what must fail the build

`analyzer: errors:` re-ranks any diagnostic to `error`, `warning`, `info`, or
`ignore`:
...
default; pass `--fatal-infos` when info-level diagnostics must stop CI too.
Promote a rule here instead of relying on a reviewer to notice it.
...
`page_width` moves the wrap column; the default is 80.
`trailing_commas: preserve` keeps a construct split wherever the author left a
trailing comma. Under the default, the formatter removes that comma and joins
the construct whenever it fits on one line.

## Silence narrowly, with the reason
...
Use `// ignore_for_file:` only for a file that is exempt as a whole. Never
disable a rule project-wide to silence one call site. A suppression with no
reason gets deleted at the next review, because nobody can tell whether it still
applies.
