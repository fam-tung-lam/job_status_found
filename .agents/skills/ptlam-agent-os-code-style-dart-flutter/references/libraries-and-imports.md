...
file. Keep a sealed root and its direct subtypes in the same library.
...
With no generated output, a new split becomes a separate library with its own
imports. Where an existing part remains, use the string form,
`part of 'orders.dart';`, which `use_string_in_part_of_directives` enforces.
