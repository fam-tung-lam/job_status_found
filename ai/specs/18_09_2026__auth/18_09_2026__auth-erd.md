...
all eight tables, revision `0002` drops `users.terms_version` and
`users.terms_accepted_at` until terms acceptance returns as the last MVP
change, and `features/auth/infrastructure/db/tables/` maps them, one
...
- Only surrogate keys have a database default. The application sets every
  other value, and its injected `utc_now` sets every instant, so tests control
  time.
...
erDiagram
    ...
    REFRESH_TOKEN["refresh_tokens"] {
        ...
        timestamptz revoked_at "nullable; grace-superseded child or session/family end"
    }
```
...
- Normal rotation sets the exchanged parent's `used_at` and leaves its
  `revoked_at` null. A retry inside the reuse grace revokes only the unused
  child it supersedes before inserting its replacement. Session or family
  termination sets `revoked_at` on the family's tokens.
...
erDiagram
    ...
    AUTH_EVENT["auth_events"] {
        ...
        text event_type "such as email_verification_failed, sign_in_failed, or refresh_token_reused"
...
- Email confirmation counts recent `email_verification_failed` rows by user,
  so replacing an open challenge does not reset the five-pair allowance in the
  rolling verification-code lifetime.
