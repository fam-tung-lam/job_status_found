...
outlives the account without naming it.
...
erDiagram
    ...
    USER["users"] {
        uuid id PK
        text email "as the user typed it; used for display and delivery"
        ...
        text terms_version "version of the terms accepted at sign-up"
...
- Index `sessions (user_id) WHERE revoked_at IS NULL` serves the session list
  and the revoke-all operations.
