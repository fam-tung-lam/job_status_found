...
Submitting the right code to `POST /v1/auth/email-verification/confirm` marks
the email verified and returns a `TokenPair`. A `web` client receives its
refresh token only as an `HttpOnly` cookie.
...
- `ConfirmEmailVerificationUseCase` and its endpoint. The request carries the
  email, the code, `client_kind`, and `remember_me`.
...
- The right code within 15 minutes sets `email_verified_at`, consumes the
  challenge, creates one session and one refresh token, and returns 200
  `TokenPair` (§6.1, §7).
...
- The access token's header and claims match §5, and it expires 15 minutes
  after issue.
- A wrong code returns 400 `verification_code_invalid` and increments
  `attempt_count`. After the fifth wrong code, the right code also fails
  (§6.1, §7).
- An expired code, a consumed code, and an email with no open challenge return
  the same `verification_code_invalid` (§7, §9).
...
- Request field names besides `client_kind` and `remember_me`, within
  `snake_case` (§7).
