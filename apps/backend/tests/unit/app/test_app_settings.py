...
def test_database_password_is_absent_by_default(monkeypatch: pytest.MonkeyPatch) -> None:
    # Given: no password in the environment.
    monkeypatch.delenv("JSF_DATABASE_PASSWORD", raising=False)

    # When: the settings are loaded without a `.env` file.
    settings = AppSettings(_env_file=None)

    # Then: no password is configured.
