...
class TestAppSettings:
    """Loading `AppSettings` from `JSF_*` environment variables."""

    def test_database_settings_come_from_prefixed_environment_variables(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """
        Given: the database environment Compose gives the backend container.
        When: the settings are loaded without a `.env` file.
        Then: the database fields hold those values, and the password is masked when printed.
        """
    ...
    def test_an_empty_optional_setting_means_its_default(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """
        Given: optional SMTP credentials that Compose passes as empty strings.
        When: the settings are loaded.
        Then: no SMTP credentials are configured.
        """
        # Given: optional SMTP credentials that Compose passes as empty strings.
    ...
    def test_smtp_credentials_on_an_unprotected_connection_are_refused(
        ...
        When: the settings are loaded.
        Then: loading fails instead of leaking the credentials.
        """
        # Given: SMTP credentials with a connection that sends them in the clear.
