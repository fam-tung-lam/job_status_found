...
class TestSmtpEmailSenderClient:
    """The client against a real network connection that fails."""

    @pytest.mark.asyncio
    async def test_an_unreachable_mail_server_fails_without_naming_the_recipient(self) -> None:
        """
        Given: a client pointed at a local port where no SMTP server listens.
        When: an email is sent.
        Then: the send fails with `EmailDeliveryFailure`, which names neither recipient nor content.
        """
        # Given: a client pointed at a local port where no SMTP server listens.
